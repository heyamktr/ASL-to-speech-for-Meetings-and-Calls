...
window.addEventListener('message', async (event) => {
  ...
  holistic.onResults((results: any) => {
    ...
    // Add pose landmarks (33 landmarks × 4: x, y, z, visibility)
    if (results.poseLandmarks?.length) {
      results.poseLandmarks.forEach((lm: any) => {
        landmarks.push(lm.x, lm.y, lm.z, lm.visibility);
      });
    } else {
      landmarks.push(...Array(132).fill(0)); // 33 landmarks × 4
    }

    // Add left hand landmarks (21 landmarks × 3: x, y, z)
    if (results.leftHandLandmarks?.length) {
      results.leftHandLandmarks.forEach((lm: any) => {
        landmarks.push(lm.x, lm.y, lm.z);
      });
    } else {
      landmarks.push(...Array(63).fill(0)); // 21 landmarks × 3
    }

    // Add right hand landmarks (21 landmarks × 3: x, y, z)
